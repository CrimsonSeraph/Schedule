...
- `MyUI`（QML 模块，提供 MainDesktop.qml / MainMobile.qml）
- `MyEngine`（桥接层，提供 AppBridge 类型）
...
自动化自检：以 `-DBUILD_SELFTEST=ON` 配置后运行 `MyApp.exe --selftest`，程序会自动点击“测试”按钮并退出。点击界面中的“测试”按钮，控制台 / 调试输出应出现：
