...
- 正方教务（zfn / zfsoft V9）页面解析器：`data/import_export/academic_affairs/`，含自带的 GBK 解码链路（Qt 6 不内置 GBK 编解码器）。
- 「从教务导入」：内嵌浏览器（Qt WebView → Qt WebEngine → 系统浏览器兜底）打开教务系统，抓取当前页面并把原文交给 `ImportManager`，与文件导入共用预览 / 冲突检测 / 合并策略。
- 安徽工程大学适配（`ahpu-jwxt`）：只配登录页的「浏览器直达入口」，登录后抓取课表页。
- 导入向导与设置页均改为面向用户的入口列表；接口地址与 Cookie 移入默认折叠的高级选项。
